...
import { X, Plus, Minus, Check, ShoppingBag, Layers, ArrowRight } from 'lucide-react';
import { FoodItem, ProteinOption } from '../types';
import { PROTEIN_OPTIONS } from '../constants';
...
const MultiOrderModal: React.FC<MultiOrderModalProps> = ({ isOpen, onClose, item, onConfirm }) => {
    const [step, setStep] = useState<'choice' | 'multiple'>('choice');
    ...
    const toggleProtein = (selectionId: number, protein: ProteinOption) => {
        setSelections(prev => prev.map(s => {
            if (s.id === selectionId) {
                const alreadySelected = s.proteins.find(p => p.id === protein.id);
                return {
                    ...s,
                    proteins: alreadySelected
                        ? s.proteins.filter(p => p.id !== protein.id)
                        : [...s.proteins, protein]
                };
            }
            return s;
                ...
                <div className="p-8 border-b border-stone-100 flex items-center justify-between bg-orange-950 text-white shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-300">Custom Atmos Sync</p>
                        </div>
                    </div>
                    ...
                    {step === 'choice' ? (
                        <div className="space-y-8 py-4">
                            <div className="text-center space-y-4">
                                ...
                                <p className="text-stone-400 font-medium text-base leading-relaxed">Choose your portion size for this delicacy.</p>
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <button
                                    onClick={() => {
                                        setQuantity(1);
                                        setSelections([{ id: 0, proteins: [] }]);
                                        setStep('multiple');
                                    }}
                                    className="group p-8 rounded-[2.5rem] border-2 border-stone-100 bg-stone-50 hover:bg-white hover:border-orange-400 hover:shadow-xl transition-all text-left space-y-4"
                                >
                                    <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-orange-950 shadow-sm group-hover:bg-orange-100 transition-colors">
                                        <ShoppingBag size={28} />
                                    </div>
                                    <div>
                                        <h4 className="font-serif font-black text-xl text-orange-950">Just 1 Plate</h4>
                                        <p className="text-stone-400 text-sm font-medium">Standard delicacy portion.</p>
                                    </div>
                                </button>
                                ...
                                >
                                    ...
                                    <div>
                                        <h4 className="font-serif font-black text-xl text-orange-950">Multiple Plates</h4>
                                        <p className="text-stone-400 text-sm font-medium">Custom bulk pack for the group.</p>
                                    </div>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-10 animate-reveal-up">
                            ...
                            <div className="flex flex-col items-center gap-6 pb-10 border-b border-stone-100">
                                ...
                                <div className="flex items-center gap-8 bg-orange-50 p-2 rounded-3xl border border-orange-100 shadow-inner">
                                    <button
                                        ...
                                        className="w-14 h-14 flex items-center justify-center bg-white rounded-2xl border border-stone-100 text-stone-400 hover:text-red-500 transition-all active:scale-90"
                                    >
                                        <Minus size={24} />
                                    </button>
                                    ...
                                    <button
                                        onClick={() => handleQuantityChange(quantity + 1)}
                                        className="w-14 h-14 flex items-center justify-center bg-orange-950 text-white rounded-2xl shadow-lg hover:bg-black transition-all active:scale-90"
                                    >
                                        <Plus size={24} />
                                    </button>
                                </div>
                            ...
                            {/* Protein Customization per Plate */}
                                    ...
                                    <div key={selection.id} className="bg-stone-50 rounded-[2rem] p-8 border border-stone-100 space-y-6">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {PROTEIN_OPTIONS.map(protein => {
                                                const isActive = selection.proteins.find(p => p.id === protein.id);
                                                return (
                                                    <button
                                                        key={protein.id}
                                                        onClick={() => toggleProtein(selection.id, protein)}
                                                        className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isActive ? 'bg-orange-100 border-orange-400' : 'bg-white border-stone-100 hover:border-orange-200 shadow-sm'}`}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-5 h-5 rounded-md flex items-center justify-center border transition-all ${isActive ? 'bg-orange-950 border-orange-950 text-white' : 'bg-white border-stone-300'}`}>
                ...
                {step === 'multiple' && (
                    <div className="p-8 border-t border-stone-100 bg-stone-50/50 shrink-0">
                        <button
                            ...
                            className="w-full bg-orange-400 hover:bg-black text-white py-6 rounded-[1.5rem] font-black flex items-center justify-center gap-4 shadow-xl transition-all active:scale-95 text-base uppercase tracking-[0.2em]"
                        >
